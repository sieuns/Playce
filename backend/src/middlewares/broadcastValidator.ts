...
export const createBroadcastValidator = [
  body("store_id")
    .notEmpty().withMessage("식당을 입력해주세요.")
    .isInt({ min: 1 }).withMessage("식당은 1 이상의 정수여야 합니다."),
  body("sport_id")
    .notEmpty().withMessage("스포츠를 입력해주세요.")
    .isInt({ min: 1 }).withMessage("스포츠는 1 이상의 정수여야 합니다."),
  body("league_id")
    .notEmpty().withMessage("리그를 입력해주세요.")
    .isInt({ min: 1 }).withMessage("리그는 1 이상의 정수여야 합니다."),
  body("match_date")
    .notEmpty().withMessage("경기 날짜를 입력해주세요.")
    .isISO8601().withMessage("경기 날짜를 형식에 맞게 입력해주세요. (예: 2025-07-01)"),
  body("match_time")
    .notEmpty().withMessage("경기 시간을 입력해주세요.")
    .matches(/^([01]\d|2[0-3]):([0-5]\d)$/)
    .withMessage("경기 시간을 형식에 맞게 입력해주세요. (예: 18:30)"),
  body("team_one").optional().isString().withMessage("팀1은 문자열이어야 합니다."),
  body("team_two").optional().isString().withMessage("팀2는 문자열이어야 합니다."),
  body("etc").optional().isString().withMessage("기타는 문자열이어야 합니다."),
  validate,
];
...
export const updateBroadcastValidator = [
  ...
  body("sport_id").optional()
    .isInt({ min: 1 }).withMessage("스포츠는 1 이상의 정수여야 합니다."),
  body("league_id").optional()
    .isInt({ min: 1 }).withMessage("리그는 1 이상의 정수여야 합니다."),
  body("match_date").optional()
    .isISO8601().withMessage("경기 날짜를 형식에 맞게 입력해주세요. (예: 2025-07-01)"),
  body("match_time").optional()
    .matches(/^([01]\d|2[0-3]):([0-5]\d)$/)
    .withMessage("경기 시간을 형식에 맞게 입력해주세요. (예: 18:30)"),
  body("team_one").optional()
    .isString().withMessage("팀1은 문자열이어야 합니다."),
  body("team_two").optional()
